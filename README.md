...
- **view_city_directory**: tracked when a user clicks on the view city directory button. Optionally, it can track an specific listing by passing the `listing_id` as a parameter.
- **view_experiences:** tracked when a user clicks on the view experiences button. Optionally, it can track an specific listing by passing the `experience_id` as a parameter.
